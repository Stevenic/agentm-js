...
export async function mapList<TValue extends {}>(args: MapListArgs): Promise<AgentCompletion<Array<TValue>>> {
    const { goal, list } = args;
    const temperature = args.temperature ?? 0.0;

    // Create a parallel completion function
    const completePrompt = parallelCompletePrompt<WithExplanation<TValue>>(args);
    ...
    // Enumerate list
    const useJSON = true;
    const promises: Promise<AgentCompletion<WithExplanation<TValue>>>[] = [];
...
const systemPrompt = 
`You are an expert mapping items from one type to another.
